...
def evaluate(model_path, data_path, split='TESTING'):
    ...
    matrix = confusion_matrix(labels_, predictions_)
    print(matrix)
    print(np.sum(matrix, axis=1))
    tag_list = ["ADJ", "ADP", "ADV", "AUX", "CCONJ", "DET", "INTJ", "NOUN", "NUM", "PART", "PRON", "PROPN", "PUNCT", "SCONJ", "SYM", "VERB", "X"]
    df_cm = pd.DataFrame(matrix, index=[i for i in tag_list], columns=[i for i in tag_list])
    confusion_m = plt.figure(figsize=(12, 7))
    sn.heatmap(df_cm, annot=True, fmt='g')
    confusion_m.savefig("./confusion matrix/context-mean.pdf")
    plt.show()
