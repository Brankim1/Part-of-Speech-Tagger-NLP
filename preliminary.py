...
def train(data_path):
    ...
    for sample in read_jsonl(data_path):
        ...
        for k in range(len(tokens)):
            tag_dic[tags[k]] += 1
            tokens_counts[tokens[k]][tags[k]] += 1
    '''
    Set model model params to be a dictionary with tag frequency counts
    '''
    most_frequent_tag = max(tag_dic, key=tag_dic.get)
    model_params = (tokens_counts, most_frequent_tag)
...
def predict_tag(token, params):
    ...
    YOUR CODE HERE
    '''
    training_dic, most_frequent_tag = params
    tag_pred = ""
    tag_count = 0
    # token exist in training data
    if token in training_dic:
        tag_pred = max(training_dic[token], key=training_dic[token].get)
        tag_count = training_dic[token][tag_pred]
    # token does not exist in training data
    else:
        tag_pred = most_frequent_tag
        tag_count = 0
    return tag_pred, tag_count  # return tag_pred, tag_count
